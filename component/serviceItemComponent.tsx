...
export default function ServiceItemComponent({children, icon, img, title, hasButtons = false, hasHoverAnimation = false, iconGradiant}:propsType):ReactNode {
    ...
    return (
        <div className="lg:h-[528px] h-[555px] overflow-hidden rounded-[40px] relative group">
            <div className={`lg:absolute relative top-0 left-0 w-full transition-all ease-in-out duration-1000 z-[-1] ${(hasHoverAnimation) ? 'group-hover-skew' : ''}`}>
            ...
            <div className={`transition-all ease-in-out duration-1000 h-full w-full absolute top-0 left-0 p-[44px] flex items-end justify-start origin-right ${(hasHoverAnimation) ? 'group-hover-skew-small-scale' : ''}`}>
