...
export default function CircleProgressComponent({percentage, theme, size = 40}:propsType):ReactNode {
    // Returning JSX
    return (
        <div data-size={size} className="aspect-square data-[size='40']:lg:w-[40%] data-[size='40']:w-[30%] data-[size='full']:w-full rounded-full shadow-xl relative p-[5px] flex bg-lightGrey">
            <div 
                className="rounded-full absolute w-full h-full z-[1] top-0 left-0" 
                style={{background: `conic-gradient(${(theme === 'blue') ? '#2B96CC' : (theme === 'green') ? '#93AF40' : (theme === 'orange') ? '#FCAA0B' : '#49B486'} ${percentage}%, #BBBBBB 0)`}} 
            />
