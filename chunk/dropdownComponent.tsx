...
export default function DropdownComponent({children, title, theme}:propsType):ReactNode {
    ...
    // Returning JSX
    return (
        <div className="shrink-0 relative lg:w-[306px] w-full lg:pr-0 pr-[60px]">
            <button 
            ...
            >
                <span className="block truncate text-current font-normal lg:text-[16px] text-[13px]">{title}</span>
                <span className="block text-current"><IconComponent size={16} name="chevron-down" /></span>
            </button>
            <div data-theme={theme}  data-opened={isOpened} className="lg:mr-0 mr-[60px] absolute transition-all z-[10] duration-500 data-[opened='true']:visible data-[opened='true']:opacity-100 data-[opened='false']:invisible data-[opened='false']:opacity-0 top-[110%] left-0 py-[10px] data-[theme='dark']:bg-lightestDark border data-[theme='dark']:border-theme w-full rounded-[16px]">
                {children}
            </div>
        </div>
    );
